...
const Home=(props)=> {
    ...
    useEffect(async() =>{
         const token = await props.token();
          await axios.get('http://localhost:8000/api/items',{
              ...
              .then(response =>{ if (response.data === null){return}else{setTodos(response.data)}})
    ...
    const handleSubmit= e =>{
        ...
        const newTodo = {datetime: new Date().toLocaleString(), text:input, completed: false}
    ...
    useEffect(()=>{if(localStorage.getItem('isAuthenticated') === 'false'){
        navigate('/login')
    }},)
    return (
        <Fragment>
